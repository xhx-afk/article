...
class BaseSolver(object):
    ...
    def load_resume_state(self, path: str, fallback_paths=None):
        """Load resume checkpoint, optionally falling back from a missing/corrupt file."""
        if path.startswith('http'):
            state = torch.hub.load_state_dict_from_url(path, map_location='cpu')
            loaded_path = path
        else:
            candidates = [Path(path)]
            candidates.extend(Path(item) for item in (fallback_paths or []))
            state = None
            loaded_path = None
            errors = []
            seen = set()
            for candidate in candidates:
                ...
                try:
                    ...
                    if candidate.stat().st_size <= 0:
                        raise EOFError(f"checkpoint 是空文件：{candidate}")
                    loaded = torch.load(candidate, map_location='cpu')
                    if not isinstance(loaded, dict) or 'model' not in loaded:
                        raise RuntimeError(f"checkpoint 缺少 model state：{candidate}")
                    state = loaded
                    loaded_path = str(candidate)
                    break
                except Exception as exc:
                    errors.append(f"{candidate}: {type(exc).__name__}: {exc}")
                    print(f"[WARN] 无法加载 checkpoint，尝试下一个候选：{errors[-1]}")
            if state is None:
                details = "\n".join(errors) if errors else "没有可用候选文件"
                raise RuntimeError(f"所有 resume checkpoint 均加载失败：\n{details}")

        # state['model'] = remove_module_prefix(state['model'])
        self.load_state_dict(state)
        print(f'Loaded resume checkpoint: {loaded_path}')
        return loaded_path
